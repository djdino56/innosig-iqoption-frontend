  ...
  methods: {
    ...
    tryToRegisterIn() {
      ...
      if (this.v$.$invalid) {
        ...
        } else if (process.env.VUE_APP_DEFAULT_AUTH === "eve") {

          let newAccount = new AccountViewModel(this.user)

          newAccount.create(['username', 'password', 'email']).then(response => {
            if (response.data._status === 'OK') {
              newAccount._etag = response.data._etag;
              newAccount._updated = response.data._updated;
              newAccount._created = response.data._updated;
              newAccount._id = response.data._id;
              Swal.fire({
                icon: 'success',
                title: 'Successful!',
            ...
            this.tryingToRegister = false;
            this.regError = error ? error.response.data._issues : "";
            this.isRegisterError = true;
          });
        }
      }
